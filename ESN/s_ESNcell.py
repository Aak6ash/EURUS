...
class ESNCell(nn.Module):
    ...
    # Constructor
    def __init__(self, input_dim, output_dim, spectral_radius=0.9, bias_scaling=0, input_scaling=1.0, w=None, w_in=None,leaky_rate=1.0,
        ...
        :param w_bias: Bias weights matrix0
        :param sparsity:
        :param input_set:
        :param w_sparsity:
        ...
        # self.leaky_rate = torch.FloatTensor([0.7])
        ...
        # Initialize leaky_rate
        # self.leaky_rate = nn.Parameter(Variable(self.leaky_rate))
    ...
    # Forward
    def forward(self, u, y=None, w_out=None, reset_state=True):    # Reset of the hidden layer is not required
        """
        Forward
        ...
        for b in range(n_batches):
            # Reset hidden layer    # Reset of the hidden layer is not required
            if reset_state:
                self.reset_hidden()
            # end if
            ...
            for t in range(time_length):
                ...
                if self.feedbacks and self.training and y is not None:
                    ...
                    # Add bias
                    bias_hidden = torch.cat((Variable(torch.ones([1], dtype=torch.float32)), self.hidden), dim=0)
    ...
    def init_hidden(self):
        ...
        return Variable(torch.zeros((self.output_dim), dtype=torch.float32), requires_grad = False)
    ...
    @staticmethod
    def generate_gaussian_matrix(size, sparsity, mean=0.0, std=1.0, dtype=torch.float64):
        """
    ...
    def generate_w(output_dim, w_distrib='uniform', w_sparsity=None, mean=0.0, std=1.0, seed=None, dtype=torch.float32):
        ...
        if w_distrib == 'uniform':
            ...
            w = ESNCell.generate_gaussian_matrix(size=(output_dim, output_dim), sparsity=w_sparsity, mean=mean, std=std, dtype=torch.float64)
        # end if
