...
class StackedESN(nn.Module):
    ...
    # Constructor
    def __init__(self, input_dim, hidden_dim, output_dim, leaky_rate=[1.0, 0.5, 1.0], spectral_radius=0.9, bias_scaling=0,
                 input_scaling=1.0, w=None, w_in=None, w_bias=None, sparsity=None, input_set=(1.0, -1.0),
                 w_sparsity=None, nonlin_func=torch.tanh, learning_algo='grad', ridge_param=0.0, with_bias=True):
        """
        Constructor
        ...
        for n in range(self.n_layers):
            ...
            # Create LiESN cell
            self.esn_layers.append(s_ESNcell.ESNCell(
                layer_input_dim, hidden_dim[n], layer_spectral_radius, layer_bias_scaling,
                layer_input_scaling, layer_w, layer_w_in, layer_leaky_rate, layer_w_bias, None, layer_sparsity, layer_input_set,
                layer_w_sparsity, layer_nonlin_func
            ))
        # end for

        # Output layer
        self.output = RRcell.RRCell(self.n_features, output_dim, ridge_param, False, with_bias, learning_algo)
    # end __init__
    ...
    def forward(self, u, y=None):
        ...
        hidden_states = hidden_states.type(torch.FloatTensor)
    ...
    def generate_ws(n_layers, reservoir_size, w_sparsity):
        ...
        :return:
        """
        ws = torch.DoubleTensor(n_layers, reservoir_size, reservoir_size)
        for i in range(n_layers):
            ws[i] = s_ESNcell.ESNCell.generate_w(reservoir_size, w_sparsity)
        # end for
        return ws
    # end for
